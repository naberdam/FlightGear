...
void openSocketForDataServerCommand(string portOfServerString) {
  ...
  int portOfServer =
      (int) VariablesSingelton::getInstanceOfVariablesSingelton()->calculateStringInInterpret(portOfServerString);
...
int OpenDataServerCommand::execute(vector<vector<std::__cxx11::string> > &detailsOfTheCommand, unsigned int index) {
  ...
  //thread for reading from server so we will be able to updating our variables
