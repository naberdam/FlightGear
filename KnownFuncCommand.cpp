...
KnownFuncCommand::KnownFuncCommand(const vector<vector<string>> &detailsOfFuncDeclaration) : detailsOfFuncDeclaration(detailsOfFuncDeclaration) {}
...
int KnownFuncCommand::execute(vector<vector<string> > &detailsOfTheCommand, unsigned int index) {
    ...
    advance(item, ++index);
    detailsOfTheCommand.insert(item, this->detailsOfFuncDeclaration.begin(), this->detailsOfFuncDeclaration.end());
